...
$u_user=mysqli_real_escape_string($conn,$_POST['user']);
$u_password=mysqli_real_escape_string($conn,md5($_POST['password']));
$u_role=mysqli_real_escape_string($conn,$_POST['role']);
$sql="SELECT username FROM user WHERE username='{$u_user}'";
$result=mysqli_query($conn,$sql) or die("query unsuccessful!");
if(mysqli_num_rows($result)>0){
  echo "<p style='color:red;text-align:center;margin:10px 0;'> username already taken</p>";
}else
{
$sql1= "INSERT INTO user (first_name,last_name,username,password,role) VALUES('{$u_fname}','{$u_lname}','{$u_user}','{$u_password}','{$u_role}')";
