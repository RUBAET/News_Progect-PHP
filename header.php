...
$page_name=basename($_SERVER['PHP_SELF']);
 switch ($page_name) {
     ...
     case 'category.php':
          if(isset($_GET['c_id'])){
            $sql_title="SELECT * FROM category WHERE category_id={$_GET['c_id']}";
            $result_title=mysqli_query($conn,$sql_title) or die("query failed.");
            $row_title=mysqli_fetch_assoc($result_title);
     ...
     case 'author.php':
         if(isset($_GET['u_id'])){
            $sql_title="SELECT * FROM user WHERE user_id={$_GET['u_id']}";
            $result_title=mysqli_query($conn,$sql_title) or die("query failed.");
            $row_title=mysqli_fetch_assoc($result_title);
                    ...
                    if(mysqli_num_rows($result)>0){
                      while($row=mysqli_fetch_assoc($result)){
                        if($row["logo"]==""){
                          ...
                          echo '<a href="index.php"><img class="logo" 
                          src="images/'.$row["logo"].'"></a>';
                            }
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <ul class='menu'>
                    ...
                    while($row=mysqli_fetch_assoc($result)){
                        if(isset($_GET['c_id']) && $_GET['c_id']==$row['category_id']){
